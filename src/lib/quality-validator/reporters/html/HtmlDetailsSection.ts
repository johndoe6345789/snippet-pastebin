...
export function generateFindingsSection(findings: Finding[]): string {
  ...
  for (const severity of severityOrder) {
    const severityFindings = grouped[severity as keyof typeof grouped] || [];
 ...
 */
export function generateRecommendationsSection(recommendations: Recommendation[]): string {
 ...
 */
export function generateRecommendationCard(recommendation: Recommendation, index: number): string {
  return `
